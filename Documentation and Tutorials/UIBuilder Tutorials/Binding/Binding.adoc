...
The concept and different aspects of bindings are explained in the paragraph "General information". It is followed by explanations on how to generate bindings in the SICK App Desiger UI Builder. At the end of the tutorial, you will find specific step-by-step examples. 

## Prerequisites
* Basic skills the SICK App Designer, mainly how to serve functions and events and how to create apps. Consider the following tutorials for more information:
...
To create a binding in the UI-Builder, the app that serves the functionality needs to be checked in the "App Model Configuration" view subsection of the SICK App Designer. If you want to bind the UI to device-specific functionalities, make sure that you are connected to the device.
...
Start by downloading the link:https://gitlab.com/sick-appspace/samples/Bindings[prepared Bindings app] and open it in the SICK App Designer. As you can see, the app consists of an html page and a lua script. When you download the app onto your device the interactive elements won't do anything.
