...
## Prerequisites
* Basic skills in using *SICK App Designer*, mainly how to connect to a device or emulator, deploy "SensorApps" onto it and open the device page. 
...
// TODO: add links to github once available
. Open the IDE SICK App Designer, start the *SICK AppEngine* and connect to it.
. Import the downloaded SensorApp
** In *SICK App Designer*: extract the zip file in the selected workspace
...
* In *SICK App Designer* there is no way of automatically adding the components to existing SensorApps as of yet. For now please follow the steps described below:
